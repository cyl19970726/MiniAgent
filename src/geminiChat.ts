...
export class GeminiChat implements IChat {
  ...
  async sendMessageStream(
    ...
    try {
      ...
      // Add system prompt at the beginning if it exists and history is empty
      if (this.systemPrompt && requestContents.length === 0) {
        const systemContent = this.convertToGeminiContent([{
          type: 'text',
          text: this.systemPrompt,
        }], 'user'); // Gemini treats system messages as user messages
        requestContents = [systemContent];
      }
